...
// Signature canvas is loaded lazily only on step 5 to avoid ChunkLoadError on ngrok / certain dev setups
// (the module chunk for react-signature-canvas was failing to load when pre-declared at top level)
...
interface LoanApplicationClientProps {
  tenantName?: string;
}

export default function LoanApplicationClient({ tenantName = 'Lending Platform' }: LoanApplicationClientProps) {
  ...
  // Lazily load the signature canvas component only when we reach step 5.
  // This prevents the ChunkLoadError for the react-signature-canvas chunk on ngrok.
  ...
  useEffect(() => {
    setIsClient(true);
    loadCurrentUserAndRole();
  }, []);

  // Load signature canvas chunk only when user reaches step 5
  useEffect(() => {
    if (currentStep === 5 && !SigCanvas) {
      import('react-signature-canvas')
        .then((mod) => {
          setSigCanvas(() => mod.default);
        })
        .catch((err) => {
          console.error('Failed to load signature canvas chunk', err);
        });
    }
  }, [currentStep, SigCanvas]);

  const loadCurrentUserAndRole = async () => {
    try {
      const { data: { user: u } } = await supabase.auth.getUser();
      setSbUser(u);
      if (u) {
        // Prefer profiles (canonical post-Clerk), fallback legacy 'users' only if needed
        let role = 'BROKER_AE';
        try {
          const { data: prof } = await supabase.from('profiles').select('role').eq('id', u.id).maybeSingle();
          if (prof?.role) role = prof.role;
          else {
            const { data: legacy } = await supabase.from('profiles').select('role').eq('id', u.id).maybeSingle();
            if (legacy?.role) role = legacy.role;
          }
        } catch {}
        setCurrentUserRole(role);
      }
    } catch (e) {
      console.warn('load user/role failed', e);
    }
  };
  ...
  // Helper to parse numbers that may have commas from formatting (e.g. "1,100,000" -> 1100000)
  // This prevents parseFloat("1,100,000") from becoming 1 (which was causing $1 loan amounts).
  const parseCleanNumber = (value: string | undefined | number): number => {
  ...
  // No longer needed as a separate function — calculation is done inline in LTV onChange with latest state.
    ...
    <div className="max-w-4xl mx-auto p-8 bg-white min-h-screen">
      ...
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold">
          {borrowerUser ? 'Borrower Loan Application' : `${tenantName} Loan Application`}
        </h1>
        {borrowerUser && <p className="text-blue-600 mt-2">Retail Application • Simplified Experience</p>}
      </div>
      ...
      <div id="loan-application-form" className="border-2 border-black rounded-3xl p-10 bg-white min-h-[650px]">
              ...
              <div>
                ...
                <select 
                  ...
                  onChange={(e) => {
                    ...
                    setForm(prev => {
                      // Auto-compute loan amount based on purchase price (for acquisition) or estimated value.
                      // Uses the *latest* state via the updater to avoid stale closures.
                      const baseValue = prev.acquisitionPurchasePrice || prev.estimatedValue;
                      const baseNum = parseCleanNumber(baseValue);
                      const ltvNum = parseFloat(newLtv) || 0;
                      const calculated = Math.round(baseNum * (ltvNum / 100));
                      return { ...prev, ltv: newLtv, loanAmount: calculated.toString() };
                    });
                  }} 
        ...
        {/* Step 4 - Entity & Owners (Hidden for Borrowers) */}
        {currentStep === 4 && !borrowerUser && (
          <div>
              ...
              <div key={index} className="mb-12 border border-gray-200 rounded-3xl p-8 bg-white">
                ...
                {borrowerUser && (
                  <div>
                    ...
                    <div className="border-2 border-dashed border-gray-400 rounded-3xl p-6 bg-gray-50">
                      {SigCanvas ? (
                        <SigCanvas ref={sigPad} penColor="black" canvasProps={{ className: 'w-full h-64 border rounded-2xl bg-white' }} />
                      ) : (
                        <div className="w-full h-64 border rounded-2xl bg-gray-100 flex items-center justify-center text-gray-500 text-sm">
                ...
                {!borrowerUser && (
                  <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-2xl text-sm text-yellow-700">
                    Company user submitting — no signature required here (borrower will sign at closing / term sheet stage).
                  </div>
                )}
              </div>
            ))}

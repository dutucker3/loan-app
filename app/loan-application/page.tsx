...
export default function LoanApplicationPage() {
  ...
  const loadUserRole = async () => {
    ...
    const { data } = await supabase
      ...
      .eq('id', user.id)
      .single();
    if (data) setCurrentUserRole(data.role || 'BROKER_AE');
  };
    ...
    <div className="max-w-4xl mx-auto p-8 bg-white min-h-screen">
      ...
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold">
          {borrowerUser ? 'Borrower Loan Application' : 'CTF Funding Loan Application'}
        </h1>
                ...
                <select 
                  ...
                  onChange={(e) => {
                    ...
                    if (form.estimatedValue && newLtv) {
                      const calculated = Math.round(parseFloat(form.estimatedValue) * (parseFloat(newLtv) / 100));
                      setForm(prev => ({ ...prev, loanAmount: calculated.toString() }));
                    }
                  }} 
